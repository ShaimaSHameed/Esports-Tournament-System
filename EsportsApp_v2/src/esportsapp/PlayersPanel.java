package esportsapp;

import javax.swing.*;
import javax.swing.table.*;
import java.awt.*;
import java.sql.*;

public class PlayersPanel extends JPanel {

    private JTable table;
    private DefaultTableModel model;
    private JTextField idField, firstField, lastField, salaryField, natField, roleField, bdateField;

    private static final Color BG = new Color(100, 130, 170);
    private static final Color PANEL_BG = new Color(140, 165, 200);
    private static final Color HEADER_BG = new Color(70, 100, 140);
    private static final Color ACCENT = new Color(70, 160, 180);
    private static final Color BTN_RED = new Color(180, 70, 70);
    private static final Color BTN_BLUE = new Color(60, 100, 160);
    private static final Color BTN_GREEN = new Color(60, 140, 100);
    private static final Color TEXT = Color.WHITE;

    public PlayersPanel() {
        setLayout(new BorderLayout(10, 10));
        setBackground(BG);
        setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));

        String[] cols = {"ID", "First Name", "Last Name", "Salary", "Nationality", "Role", "Birth Date"};
        model = new DefaultTableModel(cols, 0) {
            public boolean isCellEditable(int r, int c) {
                return false;
            }
        };
        table = new JTable(model);
        styleTable(table);
        JScrollPane scroll = new JScrollPane(table);
        scroll.setBorder(BorderFactory.createLineBorder(HEADER_BG, 1));
        scroll.getViewport().setBackground(new Color(155, 180, 210));

        JPanel form = new JPanel(new GridBagLayout());
        form.setBackground(PANEL_BG);
        form.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(HEADER_BG, 1),
                BorderFactory.createEmptyBorder(12, 12, 12, 12)));

        GridBagConstraints g = new GridBagConstraints();
        g.insets = new Insets(6, 10, 6, 10);
        g.fill = GridBagConstraints.HORIZONTAL;
        g.weightx = 1.0;

        idField = addField(form, g, "Player ID:", 0, 0);
        firstField = addField(form, g, "First Name:", 0, 2);
        lastField = addField(form, g, "Last Name:", 0, 4);
        salaryField = addField(form, g, "Salary:", 1, 0);
        natField = addField(form, g, "Nationality:", 1, 2);
        roleField = addField(form, g, "Role:", 1, 4);
        bdateField = addField(form, g, "Birth Date (YYYY-MM-DD):", 2, 0);
        idField.setEditable(true);

        JPanel btnPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 8));
        btnPanel.setBackground(PANEL_BG);
        JButton load = makeButton("Load All", BTN_BLUE);
        JButton insert = makeButton("Insert", BTN_GREEN);
        JButton update = makeButton("Update", ACCENT);
        JButton delete = makeButton("Delete", BTN_RED);
        JButton clear = makeButton("Clear", new Color(100, 100, 120));
        btnPanel.add(load);
        btnPanel.add(insert);
        btnPanel.add(update);
        btnPanel.add(delete);
        btnPanel.add(clear);

        g.gridx = 0;
        g.gridy = 3;
        g.gridwidth = 6;
        g.weightx = 0;
        form.add(btnPanel, g);

        table.getSelectionModel().addListSelectionListener(e -> {
            if (e.getValueIsAdjusting()) {
                return;
            }

            int row = table.getSelectedRow();
            if (row >= 0) {

                idField.setText(safe(model.getValueAt(row, 0)));
                firstField.setText(safe(model.getValueAt(row, 1)));
                lastField.setText(safe(model.getValueAt(row, 2)));
                salaryField.setText(safe(model.getValueAt(row, 3)));
                natField.setText(safe(model.getValueAt(row, 4)));
                roleField.setText(safe(model.getValueAt(row, 5)));
                bdateField.setText(safe(model.getValueAt(row, 6)));

                idField.setEditable(false);
            }
        });
        load.addActionListener(e -> loadPlayers());
        insert.addActionListener(e -> insertPlayer());
        update.addActionListener(e -> updatePlayer());
        delete.addActionListener(e -> deletePlayer());
        clear.addActionListener(e -> clearFields());

        add(scroll, BorderLayout.CENTER);
        add(form, BorderLayout.SOUTH);
        loadPlayers();
    }

    private String safe(Object o) {
        return o == null ? "" : o.toString();
    }

    private String validateInput(boolean isUpdate) {

        String idText = idField.getText().trim();

        if (idText.isEmpty()) {
            return "Player ID is required.";
        }
        if (!idText.matches("\\d{1,4}")) {
            return "Player ID must be 1–4 digits.";
        }

        int id = Integer.parseInt(idText);

        String first = firstField.getText().trim();
        if (first.isEmpty()) {
            return "First name is required.";
        }
        if (first.length() > 25) {
            return "First name max 25 chars.";
        }

        String last = lastField.getText().trim();
        if (!last.isEmpty() && last.length() > 25) {
            return "Last name max 25 chars.";
        }

        String nat = natField.getText().trim();
        if (nat.isEmpty()) {
            return "Nationality is required.";
        }
        if (nat.length() > 25) {
            return "Nationality max 25 chars.";
        }

        String role = roleField.getText().trim();
        if (!role.isEmpty() && role.length() > 25) {
            return "Role max 25 chars.";
        }

        String salaryText = salaryField.getText().trim();
        if (!salaryText.isEmpty()
                && !salaryText.matches("\\d{1,6}(\\.\\d{1,2})?")) {
            return "Invalid salary format.";
        }

        String bdate = bdateField.getText().trim();
        if (bdate.isEmpty()) {
            return "Birth date required.";
        }
        if (!bdate.matches("\\d{4}-\\d{2}-\\d{2}")) {
            return "Birth date must be YYYY-MM-DD.";
        }

        if (!isUpdate) {
            try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(
                    "SELECT COUNT(*) FROM player WHERE player_id=?")) {

                ps.setInt(1, id);
                ResultSet rs = ps.executeQuery();

                if (rs.next() && rs.getInt(1) > 0) {
                    return "Player ID already exists.";
                }

            } catch (Exception e) {
                return "Database error checking ID.";
            }
        }

        return null;
    }

    private JTextField addField(JPanel p, GridBagConstraints g, String label, int row, int col) {
        g.gridx = col;
        g.gridy = row;
        g.gridwidth = 1;
        g.weightx = 0;
        JLabel lbl = new JLabel(label);
        lbl.setForeground(TEXT);
        lbl.setFont(new Font("Arial", Font.BOLD, 12));
        p.add(lbl, g);
        g.gridx = col + 1;
        g.weightx = 1.0;
        JTextField f = new JTextField(12);
        f.setBackground(new Color(170, 195, 220));
        f.setForeground(TEXT);
        f.setCaretColor(TEXT);
        f.setPreferredSize(new Dimension(150, 28));
        f.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(HEADER_BG),
                BorderFactory.createEmptyBorder(3, 6, 3, 6)));
        p.add(f, g);
        return f;
    }

    private JButton makeButton(String text, Color color) {
        JButton b = new JButton(text);
        b.setBackground(color);
        b.setForeground(Color.WHITE);
        b.setFont(new Font("Arial", Font.BOLD, 12));
        b.setFocusPainted(false);
        b.setBorder(BorderFactory.createEmptyBorder(7, 18, 7, 18));
        b.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        return b;
    }

    private void styleTable(JTable t) {
        t.setBackground(new Color(155, 180, 210));
        t.setForeground(TEXT);
        t.setSelectionBackground(new Color(70, 160, 180));
        t.setSelectionForeground(Color.WHITE);
        t.setRowHeight(30);
        t.setFont(new Font("Arial", Font.PLAIN, 13));
        t.setGridColor(new Color(100, 130, 170));
        t.setShowGrid(true);
        JTableHeader h = t.getTableHeader();
        h.setBackground(HEADER_BG);
        h.setForeground(Color.WHITE);
        h.setFont(new Font("Arial", Font.BOLD, 13));
        h.setPreferredSize(new Dimension(0, 35));
    }

    private void loadPlayers() {
        model.setRowCount(0);
        try (Connection con = DBConnection.getConnection(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(
                "SELECT player_id, first_name, last_name, salary, nationality, role, bdate FROM player ORDER BY player_id")) {
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt(1), rs.getString(2), rs.getString(3),
                    rs.getDouble(4), rs.getString(5), rs.getString(6), rs.getDate(7)});
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage());
        }
    }

    private void insertPlayer() {
        String error = validateInput(false);
        if (error != null) {
            JOptionPane.showMessageDialog(this, error, "Validation Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(
                "INSERT INTO player VALUES (?,?,?,?,?,?,TO_DATE(?,'YYYY-MM-DD'))")) {

            String last = lastField.getText().trim();
            String salaryText = salaryField.getText().trim();

            ps.setInt(1, Integer.parseInt(idField.getText().trim()));
            ps.setString(2, firstField.getText().trim());

// last name (nullable)
            ps.setString(3, last.isEmpty() ? null : last);

// salary (nullable)
            if (salaryText.isEmpty()) {
                ps.setNull(4, java.sql.Types.DOUBLE);
            } else {
                ps.setDouble(4, Double.parseDouble(salaryText));
            }

            ps.setString(5, natField.getText().trim());
            ps.setString(6, roleField.getText().trim());
            ps.setString(7, bdateField.getText().trim());
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "Player inserted successfully!");
            loadPlayers();
            clearFields();
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage());
        }
    }

    private void updatePlayer() {
        String error = validateInput(true);
        if (error != null) {
            JOptionPane.showMessageDialog(this, error, "Validation Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(
                "UPDATE player SET first_name=?, last_name=?, salary=?, nationality=?, role=?, bdate=TO_DATE(?,'YYYY-MM-DD') WHERE player_id=?")) {

            String last = lastField.getText().trim();
            String salaryText = salaryField.getText().trim();

            // 1. first_name
            ps.setString(1, firstField.getText().trim());

            // 2. last_name (nullable)
            ps.setString(2, last.isEmpty() ? null : last);

            // 3. salary (nullable)
            if (salaryText.isEmpty()) {
                ps.setNull(3, java.sql.Types.DOUBLE);
            } else {
                ps.setDouble(3, Double.parseDouble(salaryText));
            }

            // 4. nationality
            ps.setString(4, natField.getText().trim());

            // 5. role
            ps.setString(5, roleField.getText().trim());

            // 6. birth date
            ps.setString(6, bdateField.getText().trim());

            // 7. WHERE player_id
            ps.setInt(7, Integer.parseInt(idField.getText().trim()));

            ps.executeUpdate();

            JOptionPane.showMessageDialog(this, "Player updated successfully!");
            loadPlayers();
            clearFields();

        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage());
        }
    }

    private void deletePlayer() {
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(
                "DELETE FROM player WHERE player_id=?")) {
            ps.setInt(1, Integer.parseInt(idField.getText()));
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "Player deleted successfully!");
            loadPlayers();
            clearFields();
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage());
        }
    }

    private void clearFields() {
        idField.setText("");
        firstField.setText("");
        lastField.setText("");
        salaryField.setText("");
        natField.setText("");
        roleField.setText("");
        bdateField.setText("");

        table.clearSelection();

       
        idField.setEditable(true);
    }
}
